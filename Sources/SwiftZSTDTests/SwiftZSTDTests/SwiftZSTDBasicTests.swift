...
class SwiftZSTDBasicTests: XCTestCase {
    ...
    func testWithDictionary() {
        ...
        if let dictData = getDictionary() {
            let origData = Data([123, 231, 132, 100, 20, 10, 5, 2, 1])
    ...
    private func getDictionary() -> Data? {
        var samples = [Data]()
        samples.append(Data(Array(10...250)))
        samples.append(Data(Array(repeating: 123, count: 100_000)))
        samples.append(Data([1,3,4,7,11]))
        samples.append(Data([0,0,1,1,5,5]))
        samples.append(Data(Array(100...240)))
        samples.append(Data(Array(repeating: 230, count: 100_000)))
        samples.append(Data([10,30,40,70,110]))
        samples.append(Data([10,20,10,1,15,50]))
