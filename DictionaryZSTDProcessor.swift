...
class DictionaryZSTDProcessor
{
     ...
     */
    init?(withDictionary: Data, andCompressionLevel: Int32)
    {
    ...
    deinit {
        if (compDict != nil) { ZSTD_freeCDict(compDict) }
        if (decompDict != nil) { ZSTD_freeDDict(decompDict) }
    }
     ...
     */
    func compressBufferUsingDict(_ dataIn : Data) throws -> Data
    {
     ...
     * work, but with questionable performance.
     */
    func decompressFrameHelper(dst : UnsafeMutableRawPointer,
                               dstCapacity : Int,
